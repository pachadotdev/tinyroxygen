...
build_topic_rd <- function(topic_key, blocks, param_index = list(), family_index = list(), markdown_default = FALSE) {
  ...
  for (b in blocks) {
    ...
    if (!is.null(alias) && !is.na(alias) && nzchar(alias)) {
      aliases <- c(aliases, format_rd_alias(alias))
    }
    alias_tag <- tag_value(b$tags, "aliases")
    if (!is.null(alias_tag)) {
      alias_values <- strsplit(alias_tag, "[ \t]+")[[1]]
      aliases <- c(aliases, vapply(alias_values, format_rd_alias, character(1)))
    }
  ...
  lines <- c(
    ...
    sprintf("\\name{%s}", topic_key),
    sprintf("\\alias{%s}", vapply(aliases, format_rd_alias, character(1))),
